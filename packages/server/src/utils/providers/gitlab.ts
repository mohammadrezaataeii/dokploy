...
export const refreshGitlabToken = async (gitlabProviderId: string) => {
	...
	const response = await fetch("https://gitlab.raymand.net/oauth/token", {
		method: "POST",
		headers: {
...
export const cloneGitlabRepository = async (
	...
	const repoclone = `gitlab.raymand.net/${gitlabPathNamespace}.git`;
...
export const getGitlabRepositories = async (gitlabId?: string) => {
	...
	const response = await fetch(
		`https://gitlab.raymand.net/api/v4/projects?membership=true&owned=true&page=${0}&per_page=${100}`,
		{
			headers: {
...
export const getGitlabBranches = async (input: {
	...
	const branchesResponse = await fetch(
		`https://gitlab.raymand.net/api/v4/projects/${input.id}/repository/branches`,
		{
			headers: {
				Authorization: `Bearer ${gitlabProvider.accessToken}`,
			},
